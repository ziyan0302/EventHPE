...
def test(args):
    ...
    mpjpe_result = mpjpe_result.cpu().detach().numpy()[20*8:21*8]
    pampjpe_result = pampjpe_result.cpu().detach().numpy()[20*8:21*8]
    ...
    # Draw vertical lines every 8 units along the x-axis
    for i in range(0, int(x[-1]), 8):
        axes[0,0].axvline(x=i, color='red', linestyle='--', linewidth=0.8)
    ...
    for i in range(0, int(x[-1]), 8):
        axes[0,1].axvline(x=i, color='red', linestyle='--', linewidth=0.8)
    ...
    for i in range(0, int(x[-1]), 8):
        axes[1,0].axvline(x=i, color='red', linestyle='--', linewidth=0.8)
    ...
    for i in range(0, int(x[-1]), 8):
        axes[1,1].axvline(x=i, color='red', linestyle='--', linewidth=0.8)

    # Plot pa_mpjpe from HMR
    axes[2,0].plot(x, pampjpeHMR_result, color='blue')
    axes[2,0].set_title("hmr_pampjpe")
    axes[2,0].set_xlabel("index")
    axes[2,0].set_ylabel("hmr_pampjpe")
    for i in range(0, int(x[-1]), 8):
        axes[2,0].axvline(x=i, color='red', linestyle='--', linewidth=0.8)

    # Plot mpjpe from HMR
    axes[2,1].plot(x, mpjpeHMR_result, color='blue')
    axes[2,1].set_title("hmr_mpjpe")
    axes[2,1].set_xlabel("index")
    axes[2,1].set_ylabel("hmr_mpjpe")
    for i in range(0, int(x[-1]), 8):
        axes[2,1].axvline(x=i, color='red', linestyle='--', linewidth=0.8)

    # Plot pa_mpjpe from HMR
    axes[3,0].plot(x, pampjpeHMRNorm_result, color='blue')
    axes[3,0].set_title("norm_hmr_pampjpe")
    axes[3,0].set_xlabel("index")
    axes[3,0].set_ylabel("norm_hmr_pampjpe")
    for i in range(0, int(x[-1]), 8):
        axes[3,0].axvline(x=i, color='red', linestyle='--', linewidth=0.8)

    # Plot mpjpe from HMR
    axes[3,1].plot(x, mpjpeHMRNorm_result, color='blue')
    axes[3,1].set_title("norm_hmr_mpjpe")
    axes[3,1].set_xlabel("index")
    axes[3,1].set_ylabel("norm_hmr_mpjpe")
    for i in range(0, int(x[-1]), 8):
        axes[3,1].axvline(x=i, color='red', linestyle='--', linewidth=0.8)
