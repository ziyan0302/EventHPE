        ...
        for iSplit in range(int(args.startWindow),int(args.endWindow)):
            ...
            for iEpoch in range(args.batch_optimization_epochs):
                ...
                for iP in range(0, numImgsInSplit-1):
                    ...
                    rough_constraint_for_feat = init_joints2d[startImg + iP+1] * torch.tensor([W, H]).to(device)
                    ...
                    min_distances, closest_vert_indices = findClosestPointTorch(verts2d[iP+1]*H, pCurr[feature_around_body_mask])
...
def get_args():
    ...
    parser.add_argument('--temp_loss', type=float, default=0) #80
    parser.add_argument('--cor_loss', type=float, default=0.06) #50
